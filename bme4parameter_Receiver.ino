...
void processLoRaData(String LoRaData) {
  ...
  // Temukan posisi awal dan akhir untuk setiap nilai
    ...
    if(Firebase.RTDB.setString(&fbdo,"Sensor/Temperature",value1)){
      Serial.println(); Serial.println("Temperature: " + value1 );
      Serial.print("Berhasil save ke :"+ fbdo.dataPath());
      Serial.println("(" + fbdo.dataType() + ")"); }
    else{ Serial.println("Gagal: "+ fbdo.errorReason()); }

    if(Firebase.RTDB.setString(&fbdo,"Sensor/Pressure",value2)){
      Serial.println(); Serial.println("Pressure: " + value2 );
      Serial.print("Berhasil save ke :"+ fbdo.dataPath());
      Serial.println("(" + fbdo.dataType() + ")"); }
    else{ Serial.println("Gagal: "+ fbdo.errorReason()); }

    if(Firebase.RTDB.setString(&fbdo,"Sensor/Altitude",value3)){
      Serial.println(); Serial.println("Altitude: " + value3 );
      Serial.print("Berhasil save ke :"+ fbdo.dataPath());
      Serial.println("(" + fbdo.dataType() + ")"); }
    else{ Serial.println("Gagal: "+ fbdo.errorReason()); }

    if(Firebase.RTDB.setString(&fbdo,"Sensor/Humidity",value4)){
      Serial.println(); Serial.println("Humidity: " + value4 );
      Serial.print("Berhasil save ke :"+ fbdo.dataPath());
      Serial.println("(" + fbdo.dataType() + ")"); }
    else{ Serial.println("Gagal: "+ fbdo.errorReason()); }
